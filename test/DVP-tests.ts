  ...
  describe("DVP tests", async () => {
    ...
    it("DVP from BND to Investor in same currency unit (no cashTokenExecutor)", async () => {
      ...
      await eur.connect(centralBanker).transfer(investorA,  20000000n * 10n ** 18n);

      await eur.connect(centralBanker).transfer(fakeCurvePool.getAddress(),  30000000n * 10n ** 18n);
      await usd.connect(centralBanker).transfer(fakeCurvePool.getAddress(),  30000000n * 10n ** 18n);
      ...
      await eur.connect(investorA).approve(curveExec.getAddress(), 10000000000000000n * 10n ** 18n);
